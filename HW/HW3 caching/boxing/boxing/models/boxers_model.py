...
class Boxers(db.Model):
    ...
    def get_weight_class(cls, weight: float) -> str:
        ...
        """
        if weight < 125:
            logger.error(f"Weight must be at least 125 lbs.")
            raise ValueError(f"Weight must be at least 125 lbs.")
        if weight < 135:
            return "Lightweight"
        elif weight < 147:
            return "Welterweight"
    ...
    def get_boxer_by_id(cls, boxer_id: int) -> "Boxers":
        ...
        try:
            ...
            logger.error(f"Database error retrieving boxer by ID: {e}")
            raise
    ...
    def get_boxer_by_name(cls, name: str) -> "Boxers":
        ...
        try:
            ...
            logger.error(f"Database error retrieving boxer by name: {e}")
            raise
